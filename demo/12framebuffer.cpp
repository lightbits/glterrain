...
int main()
{
	...
	program1Layout.setUniform("time",		program1.getUniformLocation("time"));
	...
	if(status != GL_FRAMEBUFFER_COMPLETE)
		gl::shutdown("Framebuffer not complete.");
	fbo.unbind();
	...
	while(glfwGetWindowParam(GLFW_OPENED) == GL_TRUE)
	{
		...
		{
			reloading = true;
			if(!fboVS.loadFromFile("data/shaders/fbopass.vert") ||
				!fboFS.loadFromFile("data/shaders/fbopass.frag"))
				glfwCloseWindow();
		...
		postEffectVbo.bind();
		postEffectIbo.bind();
		program1.enableVertexAttrib(program1Layout.getAttribLoc("position"), 2, GL_FLOAT, GL_FALSE, 4*sizeof(GLfloat), 0);
		program1.enableVertexAttrib(program1Layout.getAttribLoc("texel"), 2, GL_FLOAT, GL_FALSE, 4*sizeof(GLfloat), (char*)0 + 2*sizeof(GLfloat));
		program1.uniform(program1Layout.getUniformLoc("tex"), 0);
		program1.uniform(program1Layout.getUniformLoc("time"), time);
		rt.bind();
