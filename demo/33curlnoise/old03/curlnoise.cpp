...
bool loadComputeShader(ShaderProgram *shader, string path)
{
...
bool load()
{
	if (!shader_particle.loadAndLinkFromFile("./demo/33curlnoise/particle") ||
		!shader_color.loadAndLinkFromFile("./demo/33curlnoise/simple") ||
		!shader_sphere.loadAndLinkFromFile("./demo/33curlnoise/sphere") ||
		!shader_blur.loadAndLinkFromFile("./demo/33curlnoise/blur") ||
...
void init(Renderer &gfx, Context &ctx)
{
	...
	rt.create(ctx.getWidth(), ctx.getHeight());
...
void render(Renderer &gfx, Context &ctx, double dt)
{
	...
	gfx.setAttributeDivisor("particlePosition", 0);
	gfx.unsetAttribute("particlePosition");
	rt.end();

	gfx.setBlendState(BlendStates::Default);
	gfx.setCullState(CullStates::CullNone);
	gfx.beginCustomShader(shader_blur);
	gfx.clear(0x00000000, 1.0);
	gfx.setUniform("res", vec2(ctx.getWidth(), ctx.getHeight()));
	gfx.setUniform("tex", 0);
	rt.bindTexture(GL_TEXTURE0);
	quad.draw();
	rt.unbindTexture();
}