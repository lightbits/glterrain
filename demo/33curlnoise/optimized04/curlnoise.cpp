...
void sort(Renderer &gfx, Context &ctx)
{
	...
	if (pass >= NUM_PASSES)
		pass = 0;
	//glBindBuffer(GL_SHADER_STORAGE_BUFFER, index_buffer.getHandle());
	//uint32 *index = (uint32*)glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, NUM_PARTICLES * sizeof(uint32), GL_MAP_READ_BIT);
	//for (int i = 0; i < NUM_PARTICLES; ++i)
	//	std::cout << index[i] << " ";
	//std::cout << "\n\n";
...
void update(Renderer &gfx, Context &ctx, double dt)
{
	...
	{
		float s = 4.0f * ctx.getMouseX() / ctx.getWidth();
		mat_light = translate(0.0f, 0.0f, -1.0f - s) * rotateX(-PI / 2.0f) * rotateY(PI / 2.0f);
	}
