...
void init(Renderer &gfx, Context &ctx)
{
	...
	ambient_col = vec3(0.007, 0.015, 0.03);
...
void update(Renderer &gfx, Context &ctx, double dt)
{
	...
	if (ctx.isMousePressed(SDL_BUTTON_LEFT))
	{
		...
		p.x = clamp(p.x, -1.0f + sphere_radius, 1.0f - sphere_radius);
		p.z = clamp(p.z, -1.0f + sphere_radius, 1.0f - sphere_radius);
		sphere_pos.x = p.x;
		sphere_pos.z = p.z;
		sphere_pos.y = p.y;
	}

	float t = ctx.getElapsedTime() * 0.8f;
	emitter_pos.x = 0.5f * sin(t);
	emitter_pos.z = 0.35f * cos(t * 0.35f);
	emitter_pos.y = 0.25f + sin(t * 2.0f) * 0.2f;
	light_pos = emitter_pos;
	//sphere_pos.y = 0.2 * sin(t * 5.0f);
...
void render(Renderer &gfx, Context &ctx, double dt)
{
	...
	gfx.setUniform("model", translate(-1.0, -0.5, 0.0) * rotateZ(-PI / 2.0) * scale(0.5, 1.0, 1.0));
	plane.draw();

	// Floor
	gfx.setUniform("color", vec3(0.76f, 0.75f, 0.5f));
	gfx.setUniform("model", translate(0.0, -1.0, 0.0) * scale(1.0f));
	plane.draw();
	...
	// Back wall
	gfx.setUniform("color", vec3(0.63f, 0.06f, 0.04f));
	gfx.setUniform("model", translate(0.0, -0.5, -1.0) * rotateX(-PI / 2.0) * scale(1.0, 1.0, 0.5));
	plane.draw();
	...
	gfx.setUniform("view", mat_view);
	gfx.setUniform("model", translate(sphere_pos) * scale(sphere_radius));
	gfx.setUniform("color", vec3(0.4, 0.55, 1.0));
	gfx.setUniform("lightPos", light_pos);
	gfx.setUniform("lightColor", light_col);
	gfx.setUniform("ambientColor", ambient_col);
	sphere.draw();
