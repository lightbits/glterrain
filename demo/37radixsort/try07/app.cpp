...
const uint32 block_size = 128; // Remember to update local_size in shader
...
{
	const uint32 work_group_size = 256;
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, scan);
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, keys);
	glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3, flag);
...
void init(Renderer &gfx, Context &ctx)
{
	...
	printf("sorted: ");
	print_data(buf_sorted.getHandle(), GL_SHADER_STORAGE_BUFFER, 32);
}
